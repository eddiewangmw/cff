		...
		static function html($html, $meta, $field) {
			...
			$html .= sprintf(
				'<div class="new-files">
					...
					<a class="button button-primary gp-add-file" href="#"><strong>%s</strong></a>
				</div>',
				$field['id'],
				$i18n_more
			);
	...
	class gp_Metabox_Upload_Image extends gp_Metabox_Upload {
		...
		static function html($html, $meta, $field) {
			...
			$html .= sprintf(
				...
				<a class="button button-primary gp-add-file" href="#">%s</a>',
				$field['id'],
				$i18n_more
			);
		...
		static function img_html($image, $field) {
			...
			$src = wp_get_attachment_image_src($image, 'small-square');
			if ($src[0] != ''){ $src = $src[0]; } else { $src = trailingslashit(get_template_directory_uri()) . 'backend/images/replacement/replacement300x300.png'; }
			...
			$html .= sprintf(
				'<div id="%s-dragdrop" class="%s clearfix">
					<div class = "drag-drop-inside">
						...
						<p>%s</p>
						<p class="drag-drop-buttons"><input id="%s-browse-button" type="button" value="%s" class="button button-secondary" /></p>
					</div>
				</div>',
				$img_prefix,
				implode(' ', $classes),
...
if (!class_exists('gp_Metabox_Shortcode')) {

    class gp_Metabox_Shortcode {

        static function html($html, $meta, $field) {

            return sprintf(
                '<input type="text" value=\'%s\' onClick="this.select();" class="gp-input gp-shortcode" readonly="readonly">',
                $field['cnt']
            );
